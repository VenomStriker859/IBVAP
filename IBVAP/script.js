...
if (
  ...
  function featureVideoUrl(feature) {
    const base = feature === "face" ? "/api/face/video" : feature === "night" ? "/api/night/video" : "/api/anpr/video";
    return `${base}?t=${Date.now()}`;
  }

  async function refreshFeatureStatus() {
    try {
      const [face, night, anpr] = await Promise.all([
        apiFetch("/api/face/status"),
        apiFetch("/api/night/status"),
        apiFetch("/api/anpr/status")
      ]);
      apiFaceStatus = face;
    ...
    if (page === "Night Time Surveillance" && apiNightStatus) {
      ...
      if (video && apiNightStatus.status === "COMPLETED" && apiNightStatus.output_video) {
        if (video.dataset.jobId !== apiNightStatus.job_id) {
          video.src = featureVideoUrl("night");
          video.load();
