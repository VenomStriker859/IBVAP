...
def main(
    ...
    input_path = Path(input_video) if input_video else INPUT_VIDEO
    output_path = Path(output_video) if output_video else OUTPUT_VIDEO
    vehicle_path = Path(vehicle_model_path) if vehicle_model_path else VEHICLE_MODEL_PATH
    plate_path = Path(plate_model_path) if plate_model_path else PLATE_MODEL_PATH

    if not input_path.exists():
        raise FileNotFoundError(f"Input video not found:\n{input_path}")

    if not vehicle_path.exists():
        raise FileNotFoundError(f"Vehicle model not found:\n{vehicle_path}")

    if not plate_path.exists():
        raise FileNotFoundError(f"Plate model not found:\n{plate_path}")
    ...
    return {
        ...
        "tracked_objects": len(all_track_ids),
        "raw_plate_candidates": raw_plate_candidates,
        "valid_plate_candidates": valid_plate_candidates,
        "ocr_attempts": ocr_attempts,
        "valid_ocr_observations": valid_ocr_observations,
